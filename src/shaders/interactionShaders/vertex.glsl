// THIRD MODEL - PERLIN NOISE + SECOND MODEL
...
// // SECOND MODEL - mouse proximity, velocity and throw streght with little organic waves

// uniform vec3 uTargetLocal;
// uniform float uActive;        // now 0–1 continuous (1 = touching, <1 = approaching)
// uniform float uPullStrength;
// uniform float uPullRadius;
// uniform float uTime;
// uniform vec3 uMouseVelocity;  // ← NEW
// uniform float uThrowStrength; // ← NEW
...
//   // Influence now uses the new continuous uActive (pre-touch works!)
//   float influence = uActive * smoothstep(uPullRadius, 0.0, dist);

//   vec3 dir = dist > 1e-5 ? normalize(toTarget) : vec3(0.0);

//   // === MAIN PULL (same as before) ===
//   pos += dir * influence * uPullStrength;

//   // === NEW: Mouse velocity "throwing" effect ===
//   // When you move the mouse fast, the pulled area gets flung along with it
//   pos += uMouseVelocity * influence * uThrowStrength;

//   // === MORE ORGANIC ripple (bigger + noisier) ===
...
//   // Local-space vector from this vertex toward the cursor hit on the sphere
//   vec3 toTarget = uTargetLocal - pos;
//   float dist = length(toTarget);

//   // Strong pull when vertex is near the target point; fades with distance
//   float influence = uActive * smoothstep(uPullRadius, 0.0, dist);

//   vec3 dir = dist > 1e-5 ? normalize(toTarget) : vec3(0.0);
//   pos += dir * influence * uPullStrength;

//   // Small normal ripple for extra motion (optional)
//   float wave = sin(uTime * 6.0 + dist * 8.0) * 0.04 * influence;
//   pos += normal * wave;
