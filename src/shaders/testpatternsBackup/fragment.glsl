...
float steppedDiamondDistance(vec2 p, float stairCount, vec2 shapeScale)
{
    ...
    //  vec2 steppedP = floor(abs(p) * stairCount)/ stairCount*cos(uTime * 0.25); //animation worked well-akan
...
)
{
    ...
    // Turn distance into numbered bands
    float band = floor(d / bandWidth);
    float choice = mod(band, 4.0);// Cycle 0,1,2,0,1,2...
...
// function to rotate teh diamond:
...
void main()
{
    ...
    //  float strength = mod(vUv.y * 10.0, 1.0) * mod(vUv.x * 10.0, 1.0); //So 1.0 being the limit/divider of each repitition of vUv.y*10.0
    //  gl_FragColor = vec4(strength, strength, strength, 1.0);
    ...
    // }
    // OR ternary operator:
    // strength = strength < 0.5? 0.0 : 1.0; //this is how the step function works in ternary operation
...
//     // so any value of strength below 0.5 will be 0.0 and any value above 0.5 will be 1.0
...
// testing pattern placement based on tags
// if (rowColumnTag == 0.0) {finalPatternColors = vec3(modMix1);
// }else if (rowColumnTag == 1.0) {finalPatternColors = vec3(finalPatternColors);
// }else {
// finalPatternColors = vec3(kenteColor);
...
// create diamond shape
    vec2 p = vUv - 0.5;
    //  vec2 p = cellUv - 0.5; //coordinates inside the cell, create pattern in each cell
    ...
    // p.x *= 0.6; //scale p.x or p.y before quantization
     ...
     // float diamond = abs(p.x)*0.5 + abs(p.y);// To make it wider horizontally, make X contribute less to the distance:
   ...
   vec3 diamondDesign = steppedConcentricDiamond(
    ...
    //  vec3 kenteColor = diamondDesign;
    vec3 kenteColor;

// vec3 kenteColor = turq * steppedMask;
